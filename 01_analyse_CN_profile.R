...
write.csv(sequenza, file= "Results/sequenza.results.csv",row.names=F)

# Produce sequenza plot
png(file="Results/sequenza.png",w=2000, h=1000, res=200)
y_limit=5
len=sum(sequenza$nProbes)
twoColours=T
par(mar = c(0.5,5,5,0.5), cex = 0.4, cex.main=3, cex.axis = 2.5)
ticks=seq(0, y_limit, 1)

A_rle = list(lengths=sequenza$nProbes, values=sequenza$nA)
B_rle=list(lengths=sequenza$nProbes, values=sequenza$nB)
